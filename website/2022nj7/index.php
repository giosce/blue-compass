...
group by year
union
select sum(b.registered_voters), sum(b.ballot_cast),
sum(b.dem_votes), sum(b.rep_votes), b.year,
sum(ballot_cast)/sum(registered_voters) as turnout_pct,
sum(dem_votes)/sum(ballot_cast) as dem_pct,
sum(rep_votes)/sum(ballot_cast) as rep_pct
from state_election_results b
where b.office='US House'
group by b.year
order by year desc";
...
$sql5 = "select county, town, muni_id, ld from municipal_list where cd=".$cd_id." order by 1, 2";
...
<a href='stats.php'>Municipal electoral statistics across the district</a>
<br>
...
while($row5 = mysqli_fetch_array($query5)) {
	$county = $row5["county"];
	if($county != $cty) {
		echo "<br><b><a href='county.php?county=".$county."'>".$county."</a>:</b> ";
		$cty = $county;
	}
...
if(mysqli_num_rows($query_county_stats) > 0) {
	...
	if($cd_id == "07") {
		...
		echo "<a href='county.php?county=Hunterdon'>Hunterdon</a> ";
		echo "<a href='county.php?county=Morris'>Morris</a> ";
		echo "<a href='county.php?county=Somerset'>Somerset</a> ";
		echo "<a href='county.php?county=Sussex'>Sussex</a> ";
		echo "<a href='county.php?county=Union'>Union</a> ";
		echo "<a href='county.php?county=Warren'>Warren</a>";	
		echo "<br>";
		echo "<br>";
	}
