...
<H2><center>Democratic Party County Committees</center></H2>
...
while($committee = mysqli_fetch_array($query_committees)) {
	...
	if($seats > 0) {
		echo "<td><a href='comm_seats.php?county=".$cnty."#table'>".$committee["seats"]."</a></td>";
		echo "<td><a href='comm_seats.php?county=".$cnty."&status=Vacant#table'>".$vacant."</a></td>";
	} else {
		echo "<td>".$seats."</td>";
		echo "<td>".$vacant."</td>";
	}		
	...
	if(!empty($committee["bylaws"])) {
		echo "<td><a target='new' href='".$committee["bylaws"]."'>ByLaws</a></td>"; 
	} else {
		echo "<td></td>";
	}
