...
from candidates_new where election_year='2021'and election_type='PRI'
...
$query_counties = mysqli_query($conn2, $sql_counties);
//$query_cds = mysqli_query($conn, $sql_cds);
$query_lds =mysqli_query($conn2, $sql_lds);
$query_offices = mysqli_query($conn2, $sql_offices);
$query_slogan = mysqli_query($conn2, $sql_slogan);
$query_endorsement = mysqli_query($conn2, $sql_endorsement);
...
$url = 'https://api.open.fec.gov/v1/candidates/totals';
  $query_fields = [
		  ...
		  'api_key' => 'Im6d58rwpSZHRKaxK1NI6wyd6hXliDtUohSGQt3M',
		  'election_full' => 'true',
		  'election_year' => 2022,
  ...
  foreach($vals as $value) {
	...
	//echo $value['name'];
	//echo $value['party'];
	//echo $value['cash_on_hand_end_period'];
	
	// build a map with key the fec-id
	$fec_map[$value['candidate_id']] = $value['cash_on_hand_end_period'];
  }  
  //print_r($fec_map);
?>
...
echo "<th style='width:300px;'>Cash On Hands</th>";
