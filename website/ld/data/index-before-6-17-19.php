...
while($reg_row = mysqli_fetch_array($registrations_query)) {
	...
	echo "<td align=right>".number_format($reg_row[2], 0, ".", ",")."</td>"; // Jan/18
	if($y == 0) { // first row, Dem
		$dem_margin[0] = $reg_row[2];
	} else if($y == 1) { // second row, Rep
		$dem_margin[0] = $dem_margin[0] - $reg_row[2];
	}
	echo "<td align=right>".number_format($reg_row[3]-$reg_row[2], 0, ".", ",")."</td>"; // Change
	if($y == 0) { // first row, Dem
		$dem_margin[1] = $reg_row[3]-$reg_row[2];
	} else if($y == 1) { // second row, Rep
		$dem_margin[1] = $dem_margin[1] - ($reg_row[3]-$reg_row[2]);
	}
	echo "<td align=right>".number_format($reg_row[3], 0, ".", ",")."</td>"; // Nov 18
	if($y == 0) { // first row, Dem
		$dem_margin[2] = $reg_row[3];
	} else if($y == 1) { // second row, Rep
		$dem_margin[2] = $dem_margin[2] - $reg_row[3];
	}

	echo "<td align=right>".number_format($reg_row[4]-$reg_row[3], 0, ".", ",")."</td>"; // Change
	if($y == 0) { // first row, Dem
		$dem_margin[3] = $reg_row[4]-$reg_row[3];
	} else if($y == 1) { // second row, Rep
		$dem_margin[3] = $dem_margin[3] - ($reg_row[4]-$reg_row[3]);
	}
	echo "<td align=right>".number_format($reg_row[4], 0, ".", ",")."</td>"; // Jan 19
	if($y == 0) { // first row, Dem
		$dem_margin[4] = $reg_row[4];
	} else if($y == 1) { // second row, Rep
		$dem_margin[4] = $dem_margin[4] - $reg_row[4];
	}

	echo "<td align=right>".number_format($reg_row[5]-$reg_row[4], 0, ".", ",")."</td>"; // Change
	if($y == 0) { // first row, Dem
		$dem_margin[5] = $reg_row[5]-$reg_row[4];
	} else if($y == 1) { // second row, Rep
		$dem_margin[5] = $dem_margin[5] - ($reg_row[5]-$reg_row[4]);
	}
	
	echo "<td align=right>".number_format($reg_row[5], 0, ".", ",")."</td>"; 	// Apr 2019
	if($y == 0) { // first row, Dem
		$dem_margin[6] = $reg_row[5];
	} else if($y == 1) { // second row, Rep
		$dem_margin[6] = $dem_margin[6] - $reg_row[5];
	}

	// Jun 2019
	echo "<td align=right>".number_format($reg_row[6]-$reg_row[5], 0, ".", ",")."</td>"; // Change
	if($y == 0) { // first row, Dem
		$dem_margin[7] = $reg_row[6]-$reg_row[5];
	} else if($y == 1) { // second row, Rep
		$dem_margin[7] = $dem_margin[7] - ($reg_row[6]-$reg_row[5]);
	}
	
	echo "<td align=right>".number_format($reg_row[6], 0, ".", ",")."</td>"; 	// Month
	if($y == 0) { // first row, Dem
		$dem_margin[8] = $reg_row[6];
		$last_values["Dem"] = $reg_row[6];
	} else if($y == 1) { // second row, Rep
		$dem_margin[8] = $dem_margin[8] - $reg_row[6];
