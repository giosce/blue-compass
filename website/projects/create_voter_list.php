...
if(is_array($_GET['precinct_id'])) {
	$sql="SELECT * FROM voter_and_voter_history where precinct_id in ('";
	foreach($_GET['precinct_id'] as $precinct){
		//echo $precinct."</br>";
		if($prec_num > 0) {
			$sql=$sql.", '";
	...
	$sql="SELECT * FROM voter_and_voter_history where precinct_id = '".$_GET['precinct_id']."'";
} else if(!empty($county_muni)) {
	$sql="SELECT * FROM voter_and_voter_history where concat(county_code, muni_code) = '".$county_muni."'";
}

//$sql="SELECT * FROM voter_and_voter_history where 1=1";
//$sql="SELECT * FROM dem_anag_and_voting_hist_union where 1=1";
$sql="SELECT * FROM anag_and_voting_hist_union where 1=1";
...
if (!empty($has_voted)) {
	$has_voted_plus_one = $has_voted + 1;
	$sql = $sql." and ((2014G + 2015G + 2016G + 2017G) = ".$has_voted." OR (2014G + 2015G + 2016G + 2017G) = ".$has_voted_plus_one.")";
}
...
$elections = false;
if(!empty($E2018) OR !empty($E2017) OR !empty($E2016)) {
	$sql = $sql." and (";
	$elections = true;
}
if(!empty($E2018)) {
...
if(!empty($E2017)) {
	...
	$sql = $sql." ".$_GET["2018-AND-OR"]." 2017G = ".$E2017;
}
if(!empty($E2016)) {
	if($E2016 == 2) {
		$E2016 = 0;
	}
	$sql = $sql." ".$_GET["2017-AND-OR"]." 2016G = ".$E2016;
}
...
$sql = $sql." order by town, ward, precinct, street_name, street_num, last_name, first_name";
...
if(is_array($_GET['precinct_id'])) {
	$sql2="SELECT * FROM voter_and_voter_history where precinct_id in ('";
	foreach($_GET['precinct_id'] as $precinct){
		//echo $precinct."</br>";
		if($prec_num > 0) {
			$sql2=$sql2.", '";
	...
	$sql2="SELECT * FROM voter_and_voter_history where precinct_id = '".$_GET['precinct_id']."'";
} else if(!empty($county_muni)) {
	$sql2="SELECT * FROM voter_and_voter_history where concat(county_code, muni_code) = '".$county_muni."'";
}
...
<th>2013</th><th>2014</th><th>2015</th><th>2016</th><th>2017</th><th>2018</th>
